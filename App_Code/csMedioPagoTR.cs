...
public class csMedioPagoTR
{
    ...
    public void GetByVcaId(int _vca_id)
    {
        ...
        try
        {
            ...
            {
               fechaDocto = dr[0].ToString();
               importe = int.Parse(dr[1].ToString());
               banco = int.Parse(dr[2].ToString());
               vca_id = int.Parse(dr[3].ToString());
               numero_transaccion = int.Parse(dr[4].ToString());
